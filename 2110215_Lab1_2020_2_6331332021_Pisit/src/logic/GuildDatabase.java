...
public class GuildDatabase {
	...
	public boolean createDepartment(String name) {
		...
		if (isExists(name))
			return false;
		Department addDepartment= new Department(name);
		myDepartments.add(addDepartment);
		return true;
	}
	...
	public boolean isExists(String name) {
		...
		//If not, return false.
		for (Department x : myDepartments) {
	...
	public ArrayList<GuildMember> removeDepartment(int index){
		...
		Department removed = getDepartment(index);
		myDepartments.remove(index);
		return removed.getAllMembers();
	}
